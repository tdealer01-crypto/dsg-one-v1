...
function blocked(verdict, auditId) {
    return {
...
export const dsgTools = [
    {
        ...
        handler: async ({ category, search }) => {
            ...
            if (gate.verdict === "BLOCK")
                return blocked("BLOCK", gate.auditId);
            if (gate.verdict === "REVIEW")
                return { content: [{ type: "text", text: `REVIEW: marketplace access requires more evidence (auditId: ${gate.auditId})` }] };
    ...
    },
    {
        ...
        handler: async ({ templateId }) => {
            ...
            if (gate.verdict !== "ALLOW")
                return blocked(gate.verdict, gate.auditId);
    ...
    },
    {
        ...
        handler: async ({ question, max_depth }) => {
