...
import type { AppBuilderApprovedPlan, AppBuilderGateResult, AppBuilderProposedPlan } from './model';
...
export function createAppBuilderPlanHash(input: {
  ...
  return hashAppBuilderObject({ proposedPlan: input.proposedPlan, gateResult: input.gateResult });
}
...
export function approveAppBuilderPlan(input: {
  ...
  if (input.decision !== 'APPROVE') throw new Error('APP_BUILDER_PLAN_NOT_APPROVED');
  if (input.gateResult.status === 'BLOCK') throw new Error('APP_BUILDER_GATE_BLOCKED');
