...
export function createAppBuilderProposedPlan(input: {
  ...
  const steps: AppBuilderPlanStep[] = [
    ...
    {
      id: 'provision-runtime-environment',
      title: 'Provision runtime environment',
      description: 'Create the minimum real runtime environment required by the approved job before any build tool runs.',
      phase: 'DESIGN',
      riskLevel: 'HIGH',
      requiresApproval: true,
      allowedPaths: ['docs/**'],
      ...
      expectedEvidence: ['runtime-environment-manifest', 'github-branch'],
    },
    {
      id: 'implement-app',
      title: 'Implement app surface',
      description: 'Implement frontend and backend surfaces after runtime approval.',
  ...
  return {
    ...
    steps,
    allowedTools: ['file.read', 'file.write', 'file.patch', 'shell.run', 'claim.verify', 'dsg.environment.provision', 'github.branch.create'],
