...
async function loadKatzillaFdaRecalls(message: string): Promise<ExternalContextItem> {
  ...
  try {
    ...
    return {
      ...
      data: result,
      reason: result.ok ? undefined : `KATZILLA_HTTP_${result.status}`,
      evidence: ['katzilla_connector', 'x_api_key_auth', 'fda_recalls_response'],
    };
  } catch (error) {
    return {
      tool: 'katzilla_fda_recalls',
      status: 'failed',
      sourceUrl: `${firstEnv(['KATZILLA_API_BASE']) || 'https://api.katzilla.dev'}${path}`,
      reason: error instanceof Error ? error.message : 'KATZILLA_FDA_RECALLS_FAILED',
      evidence: ['katzilla_connector_failed'],
    };
  }
}
...
async function loadDirectSearchEngine(message: string): Promise<ExternalContextItem | null> {
  ...
  try {
    ...
    return { tool: 'search_engine', status: 'used', sourceUrl, data, evidence: ['http_status_ok', 'search_engine_response'] };
  } catch (error) {
    return { tool: 'search_engine', status: 'failed', sourceUrl, reason: error instanceof Error ? error.message : 'SEARCH_ENGINE_FAILED', evidence: ['http_request_failed'] };
  }
}
...
async function loadKatzillaSearchEngine(message: string): Promise<ExternalContextItem> {
  ...
  try {
    ...
    return {
      tool: 'katzilla_search',
      status: result.ok ? 'used' : 'failed',
      sourceUrl: result.url,
      data: result,
      reason: result.ok ? undefined : `KATZILLA_HTTP_${result.status}`,
      evidence: ['katzilla_connector', 'x_api_key_auth', 'search_response'],
    };
  } catch (error) {
    return {
      tool: 'katzilla_search',
      status: 'unavailable',
      sourceUrl: `${firstEnv(['KATZILLA_API_BASE']) || 'https://api.katzilla.dev'}${path}`,
      reason: error instanceof Error ? error.message : 'KATZILLA_SEARCH_NOT_CONFIGURED_OR_FAILED',
      evidence: ['katzilla_search_connector_failed'],
    };
  }
}
...
async function loadSearchEngine(message: string): Promise<ExternalContextItem> {
  ...
  if (direct) {
    return {
      ...direct,
      reason: `${direct.reason || 'DIRECT_SEARCH_FAILED'}; Katzilla fallback: ${katzilla.reason || katzilla.status}`,
      evidence: [...(direct.evidence || []), ...(katzilla.evidence || [])],
    };
  }
  ...
  return {
    tool: 'search_engine',
    status: 'unavailable',
    reason: `SEARCH_ENGINE_NOT_CONFIGURED: set DSG_SEARCH_ENGINE_URL/SEARCH_ENGINE_URL or KATZILLA_SEARCH_PATH with KATZILLA_API_KEY. Katzilla fallback: ${katzilla.reason || katzilla.status}`,
    evidence: ['missing_direct_search_url', ...(katzilla.evidence || [])],
  };
}
